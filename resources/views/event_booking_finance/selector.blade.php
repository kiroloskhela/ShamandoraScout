@extends('layouts.app', ['pageTitle' => __('Choose event to manage booking finance')])

@section('content')
    <div class="flex place-content-center">
        <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-md border-2 border-blue-300">
            <div class="mb-6 text-center">
                <h2 class="text-xl font-bold text-gray-800">{{ __('Choose event') }}</h2>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="flex justify-center">
                    <button type="button" id="go-btn"
                        class="inline-flex items-center justify-center h-12 px-8 text-sm font-medium tracking-wide rounded-full bg-blue-50 text-blue-500 hover:bg-blue-100 hover:text-blue-600 transition">{{ __('Enter') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            goBtn.addEventListener('click', function() {
                ...
                if (!seasonEventID) {
                    alert(@json(__('Choose the event first')));
                    return;
                }
