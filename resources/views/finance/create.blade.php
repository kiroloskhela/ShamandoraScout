@extends('layouts.app', ['pageTitle' => __('Add finance plan')])

@section('content')
    <div class="flex place-content-center">
        <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-5xl border-2 border-blue-300">
            <div class="mb-6 text-center">
                <h2 class="text-xl font-bold text-gray-800">{{ __('Add finance plan for an event') }}</h2>
            </div>
            ...
            <form method="POST" action="{{ route('finance.store') }}">
                ...
                <div class="space-y-6">
                    ...
                    <div id="event-info-box"
                        ...
                        <div><strong>{{ __('Event:') }}</strong> <span id="event_name_text">-</span></div>
                        <div><strong>{{ __('Event start:') }}</strong> <span id="event_start_text">-</span></div>
                        <div><strong>{{ __('Event end:') }}</strong> <span id="event_end_text">-</span></div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        ...
                        <div>
                            ...
                            <select id="allow_below_minimum_deposit" name="allow_below_minimum_deposit"
                                class="w-full h-12 px-4 border rounded-lg text-right border-slate-200 text-slate-600 focus:border-blue-500 focus:outline-none">
                                <option value="1"
                                    {{ old('allow_below_minimum_deposit', '1') == '1' ? 'selected' : '' }}>{{ __('Yes') }}</option>
                                <option value="0" {{ old('allow_below_minimum_deposit') == '0' ? 'selected' : '' }}>{{ __('No') }}</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select id="have_shirt" name="have_shirt"
                                class="w-full h-12 px-4 border rounded-lg text-right border-slate-200 text-slate-600 focus:border-blue-500 focus:outline-none">
                                <option value="1" {{ old('have_shirt', '0') == '1' ? 'selected' : '' }}>{{ __('Yes') }}</option>
                                <option value="0" {{ old('have_shirt', '0') == '0' ? 'selected' : '' }}>{{ __('No') }}</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select id="send_qr_whatsapp" name="send_qr_whatsapp"
                                class="w-full h-12 px-4 border rounded-lg text-right border-slate-200 text-slate-600 focus:border-blue-500 focus:outline-none">
                                <option value="1" {{ old('send_qr_whatsapp', '1') == '1' ? 'selected' : '' }}>{{ __('Yes') }}</option>
                                <option value="0" {{ old('send_qr_whatsapp', '1') == '0' ? 'selected' : '' }}>{{ __('No') }}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-center">
                        <button type="submit"
                            class="inline-flex items-center justify-center h-12 px-8 text-sm font-medium tracking-wide rounded-full bg-blue-50 text-blue-500 hover:bg-blue-100 hover:text-blue-600 transition">{{ __('Save finance plan') }}</button>
                    </div>
                </div>
            </form>
        </div>
                        ...
                        .then(events => {
                            ...
                            events.forEach(event => {
                                ...
                                if (parseInt(event.HasFinancePlan) === 1) {
                                    text += ' - ' + @json(__('already has a finance plan'));
                                }
