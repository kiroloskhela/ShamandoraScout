...
@section('content')
    <div class="container mx-auto px-4 py-8">
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="season_event_id" name="season_event_id"
                        class="w-full h-12 ps-4 border rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-900 {{ !empty($seasonId) ? 'text-slate-600 dark:text-slate-300' : 'text-slate-400 dark:text-slate-500' }} focus:border-blue-500 focus:outline-none"
        ...
        @if (!empty($seasonEventId) && !empty($takesReservation))
            @php
                $rc = $reservationCounts ?? ['present' => 0, 'absent' => 0, 'outside' => 0, 'unmarked' => 0, 'total' => 0];
                $attendedCount = (int) ($rc['present'] ?? 0) + (int) ($rc['outside'] ?? 0);
            @endphp
            <div class="mb-6 p-4 rounded-lg bg-amber-50 dark:bg-amber-950/30 text-amber-800 dark:text-amber-200 text-center font-semibold border border-amber-200 dark:border-amber-900">
                ...
                <div class="mt-3 flex flex-wrap items-center justify-center gap-3">
                    ...
                    <a href="{{ route('attendance.manage', ['season_id' => $seasonId, 'season_event_id' => $seasonEventId]) }}"
                        class="inline-flex items-center h-10 px-4 text-sm font-medium rounded-full border border-amber-300 dark:border-amber-800 text-amber-900 dark:text-amber-100 hover:bg-amber-100 dark:hover:bg-amber-950/50 transition">
                        {{ __('Refresh') }}
                    </a>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <div class="rounded-xl p-4 bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-200">
                    <div class="text-xs font-semibold mb-1">{{ __('Attended') }} ({{ __('Present') }} + {{ __('Outside') }})</div>
                    <div class="text-2xl font-bold">{{ $attendedCount }}</div>
                </div>
                <div class="rounded-xl p-4 bg-red-100 dark:bg-red-900/40 text-red-800 dark:text-red-200">
                    <div class="text-xs font-semibold mb-1">{{ __('Absent') }}</div>
                    <div class="text-2xl font-bold">{{ (int) ($rc['absent'] ?? 0) }}</div>
                </div>
                <div class="rounded-xl p-4 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200">
                    <div class="text-xs font-semibold mb-1">{{ __('Not scanned') }}</div>
                    <div class="text-2xl font-bold">{{ (int) ($rc['unmarked'] ?? 0) }}</div>
                </div>
                <div class="rounded-xl p-4 bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-200">
                    <div class="text-xs font-semibold mb-1">{{ __('Total booked') }}</div>
                    <div class="text-2xl font-bold">{{ (int) ($rc['total'] ?? 0) }}</div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-lg dark:border dark:border-slate-700 p-6 border-2 border-blue-300 dark:border-slate-700">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
                    <h2 class="text-lg font-semibold text-slate-800 dark:text-slate-100">{{ __('Booking attendance') }}</h2>
                    <div class="relative w-full md:w-80">
                        <input id="reservationSearch" type="text" placeholder="{{ __('Search: name / phone / ID') }}"
                        ...
                        <svg class="w-5 h-5 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" viewBox="0 0 24 24" fill="none">
                    ...
                    <table class="min-w-full border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                        <thead class="bg-slate-100 dark:bg-slate-800">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-sm font-semibold text-gray-700 dark:text-slate-200 text-right">{{ __('Type') }}</th>
                                <th class="px-4 py-2 text-sm font-semibold text-gray-700 dark:text-slate-200 text-right">{{ __('Code') }}</th>
                                <th class="px-4 py-2 text-sm font-semibold text-gray-700 dark:text-slate-200 text-center">{{ __('Status') }}</th>
                                <th class="px-4 py-2 text-sm font-semibold text-gray-700 dark:text-slate-200 text-center">{{ __('Attended') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($reservationRoster as $r)
                                @php
                                    ...
                                    $statusClass = match ($status) {
                                        ...
                                        'outside' => 'bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-200',
                                        default => 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300',
                                    };
                                    $searchHaystack = mb_strtolower(trim(($r['name'] ?? '') . ' ' . ($r['phone'] ?? '') . ' ' . ($r['code'] ?? '') . ' ' . ($r['entity_id'] ?? '')));
                                @endphp
                                <tr class="border-t border-slate-200 dark:border-slate-700 reservation-row" data-search="{{ e($searchHaystack) }}">
                                    <td class="px-4 py-3 text-right text-slate-800 dark:text-slate-100">{{ $r['name'] ?: '—' }}</td>
                                    <td class="px-4 py-3 text-right text-slate-700 dark:text-slate-200 dir-ltr">{{ $r['phone'] ?: '—' }}</td>
                                    <td class="px-4 py-3 text-right text-slate-700 dark:text-slate-200">{{ $r['booking_type_label'] }}</td>
                                    <td class="px-4 py-3 text-right text-slate-700 dark:text-slate-200 font-mono text-sm">{{ $r['code'] }}</td>
                                    <td class="px-4 py-3 text-center">
                                        <span class="inline-flex px-2.5 py-1 rounded-full text-xs font-bold {{ $statusClass }}">{{ $statusLabel }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-center text-sm font-semibold {{ !empty($r['attended']) ? 'text-green-700 dark:text-green-300' : 'text-slate-400' }}">
                                        {{ !empty($r['attended']) ? __('Yes') : __('No') }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    const rows = Array.from(document.querySelectorAll('tr.reservation-row'));
