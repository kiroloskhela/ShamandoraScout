@extends('layouts.app', ['pageTitle' => __('Upload leaders meeting document')])
...
@section('content')
    <div class="flex place-content-center">
        <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-md border-2 border-blue-300">
            ...
            <div class="mb-6 text-center">
                <h2 class="text-xl font-bold text-gray-800">رفع مستند جديد</h2>
                <p class="text-sm text-gray-500 mt-1">اختر تاريخ الاجتماع ثم ارفع الملف (PDF / DOC / DOCX)</p>
            </div>
            ...
            <form method="POST" action="{{ route('secretary.upload') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <!-- File (PDF/DOC/DOCX) -->
                    <div class="relative">
                        <label for="document_file" class="block mb-2 text-sm text-gray-700">الملف</label>
                        ...
                        {{-- Selected file name preview (basic, no JS build tools needed) --}}
    ...
    {{-- Tiny inline script to show selected file name (no build step needed) --}}
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const input = document.getElementById('document_file');
            const label = document.querySelector('label[for="document_file"]');
            const nameEl = document.getElementById('selected_file_name');

            // Allow clicking the dashed box to open the file picker (already linked by for=)
            // Update filename preview when chosen
