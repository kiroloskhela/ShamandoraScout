    ...
    <form method="POST" action="{{ route('group-person.update', $personGroupRoleRow->PersonGroupRoleID) }}">
      ...
      <div class="space-y-6">
        ...
        <div class="relative">
          ...
          <select name="group_id" id="group_id" required
            ...
            @if(optional($selectedGroup)->GroupID)
              <option value="{{ $selectedGroup->GroupID }}" selected>{{ $selectedGroup->GroupInfo }}</option>
            @else
        ...
        <div class="relative">
          ...
          <div class="flex items-center justify-between rounded-lg border border-slate-200 px-4 py-3 bg-slate-50">
          ...
          <p class="mt-2 text-xs text-gray-500">لا يمكن تغيير الشخص في شاشة التعديل. استخدم شاشة الإدخال لربط أشخاص جدد.</p>
        </div>

        <!-- الدور / المهمة -->
        <div class="relative">
          <label for="group_role_id" class="block mb-2 text-sm text-gray-700">اختر دور الشخص في المجموعة</label>

          @php
            $prefilledRoleId   = optional($selectedGroupRole)->GroupRoleID ?? optional($makhdoomGroupRole)->GroupRoleID;
            $prefilledRoleName = optional($selectedGroupRole)->GroupRoleName ?? optional($makhdoomGroupRole)->GroupRoleName;
          @endphp
          ...
          @else
            ...
            <div class="w-full h-12 px-4 flex items-center rounded-lg border border-slate-200 bg-slate-50 text-right text-slate-600">
