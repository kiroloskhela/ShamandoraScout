        ...
        <div class="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md border-2 border-red-300">
            ...
            <div class="bg-gray-50 border border-gray-100 rounded-xl p-4 mb-6 text-sm text-gray-800 space-y-2">
                <div class="flex justify-between">
                    ...
                    <span class="font-bold">{{ $trx->TransactionID }}</span>
                </div>

                <div class="flex justify-between">
                    <span class="text-gray-500">المبلغ:</span>
                    <span class="font-bold">{{ number_format($trx->Amount, 2) }}</span>
                </div>

                <div class="flex justify-between">
                    <span class="text-gray-500">التاريخ:</span>
                    <span class="font-bold">{{ \Carbon\Carbon::parse($trx->TransactionDate)->format('d/m/Y H:i') }}</span>
                </div>

                <div class="flex justify-between">
                    <span class="text-gray-500">ملاحظات:</span>
                    <span class="font-bold">{{ $trx->Notes ?? '-' }}</span>
                </div>
            </div>
