    ...
    <div class="flex place-content-center">
        <div class="bg-white dark:bg-slate-900 rounded-lg shadow-lg p-8 w-full max-w-md border-2 border-red-300 dark:border-slate-700">
            <div class="mb-6 text-center">
                <h2 class="text-xl font-bold text-gray-800 dark:text-slate-100">{{ __('Delete enrolment limit') }}</h2>
                <p class="mt-3 text-sm text-gray-600 dark:text-slate-300">
                    {{ __('Are you sure you want to delete the enrolment limit for this stage?') }}
                </p>
            </div>

            <form method="POST"
                action="{{ route('max-limits.destroy', ['id' => $selectedMarhala->QetaaID, 'sana_id' => $selectedMarhala->SanaMarhalaID]) }}"
                class="space-y-4">
                @csrf
                @method('DELETE')
