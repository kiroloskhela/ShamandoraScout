...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-6 text-center">
            <h1 class="text-2xl font-bold text-gray-800 mb-2">{{ __('Review custody request #:id', ['id' => $requestRow->RequestID]) }}</h1>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 mb-6 border-2 border-yellow-300">
                ...
                <form method="POST" action="{{ route('admin.custody_requests.approve', $requestRow->RequestID) }}">
                        ...
                        <table class="w-full text-center border border-slate-200 rounded-lg overflow-hidden">
                            <thead class="bg-slate-50">
                                <tr class="text-sm text-slate-700">
                                    ...
                                    <th class="p-3 border-b">{{ __('Note on item') }}</th>
                                </tr>
                            </thead>
                                ...
                                @foreach ($items as $idx => $it)
                                    <tr
                                        ...
                                        <td class="p-3">
                                            ...
                                            <div class="text-[11px] text-gray-500 mt-1">{{ __('Maximum: :qty', ['qty' => $it->QtyRequested]) }}
                                            </div>
                                        </td>
                                        <td class="p-3">
                    ...
                    <div class="mt-5">
                        ...
                        <textarea name="admin_note" rows="3"
                            ...
                            placeholder="{{ __('e.g. Please pick up from warehouse...') }}">{{ old('admin_note') }}</textarea>
                        <p class="mt-2 text-xs text-gray-500">{{ __('A note will be added automatically if any quantity is reduced.') }}</p>
                    </div>
            ...
            @else
                ...
                <p class="mt-4 text-xs text-gray-500">
                    {{ __('Note: Yellow rows mean the approved quantity is less than requested.') }}
                </p>
            @endif
        </div>
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 border-2 border-red-200">
                ...
                <form method="POST" action="{{ route('admin.custody_requests.reject', $requestRow->RequestID) }}">
                    ...
                    <textarea name="admin_note" rows="2"
                        ...
                        placeholder="{{ __('e.g. Items not available...') }}">{{ old('admin_note') }}</textarea>
