@extends('layouts.app', ['pageTitle' => 'مسح حالة خاصة'])

@section('content')
    <div class="flex place-content-center">
        <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-md border-2 border-red-300" dir="rtl">
            <div class="mb-6 text-center">
                <h2 class="text-xl font-bold text-gray-800">مسح حالة خاصة</h2>
            </div>

            <form method="POST" action="{{ route('personspecialcase.destroy', $case->SpecialCaseID) }}">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label class="block mb-2 text-sm text-gray-700">الملاحظة</label>
                        <textarea rows="4" readonly
                            class="w-full px-4 py-3 border rounded-lg bg-gray-50 text-right border-slate-200 text-slate-600 focus:outline-none">{{ $case->Note }}</textarea>
                    </div>
