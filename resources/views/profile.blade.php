...
<div class="profile-page -mx-2 sm:mx-0" dir="rtl" x-data="{ tab: 'personal' }">
    @if (session('success'))
        <div class="mb-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-800 text-sm font-semibold">
            {{ session('success') }}
        </div>
    @endif
            ...
            <div class="flex flex-col lg:flex-row lg:items-end gap-5 lg:gap-8">
                    ...
                    <div class="relative">
                        @if ($photoUrl)
                            <img src="{{ $photoUrl }}" alt="{{ $fullName }}"
                                class="h-32 w-28 sm:h-36 sm:w-32 rounded-2xl object-cover ring-4 ring-white shadow-lg bg-slate-100">
                        @else
                            <div class="h-32 w-28 sm:h-36 sm:w-32 rounded-2xl ring-4 ring-white shadow-lg flex items-center justify-center text-3xl font-bold text-white"
                ...
                <div class="flex-1 min-w-0 pt-2 lg:pb-2">
                    <div class="flex flex-wrap items-center gap-2 mb-1">
                        <h1 class="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">{{ $fullName }}</h1>
                        @if ($code)
                            <span class="font-mono text-sm sm:text-base text-slate-500 bg-slate-100 px-2.5 py-1 rounded-lg" dir="ltr">#{{ $code }}</span>
                        @endif
                    </div>

                    <div class="flex flex-wrap gap-2 mt-3">
                        @if (!empty($p->RotbaName))
                            <span class="inline-flex items-center rounded-full bg-indigo-50 text-indigo-700 px-3 py-1 text-xs font-bold ring-1 ring-indigo-100">
                                {{ $p->RotbaName }}
                            </span>
                        @endif
                        @if (!empty($p->QetaaName))
                            <span class="inline-flex items-center rounded-full bg-rose-50 text-rose-700 px-3 py-1 text-xs font-bold ring-1 ring-rose-100">
                                {{ $p->QetaaName }}
                            </span>
                        @endif
                        @if ($badgeName)
                            <span class="inline-flex items-center rounded-full bg-amber-50 text-amber-800 px-3 py-1 text-xs font-bold ring-1 ring-amber-100">
                                {{ $badgeName }}
                            </span>
                        @endif
                    </div>
                        ...
                        <dl class="space-y-4">
                            ...
                            <div>
                                ...
                                <dd class="text-base font-bold text-slate-900">
                                    ...
                                    @if ($emergencyLabel)
                                        <span class="text-sm font-semibold text-slate-500">({{ $emergencyLabel }})</span>
                                    @endif
                                </dd>
                            </div>
                        </dl>
                ...
                <article class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/80 overflow-hidden">
                    ...
                    @forelse ($attendance['events'] as $ev)
                        <div class="px-5 py-3.5 flex items-center justify-between gap-3 border-b border-slate-50 last:border-0">
                            <div class="min-w-0">
                                <div class="font-bold text-slate-900 truncate">{{ $ev->EventName }}</div>
                                <div class="text-xs text-slate-500 mt-0.5">{{ $ev->SeasonName }} {{ $ev->SeasonYear }} · {{ $ev->EventStartDate }}</div>
                            </div>
                            @php
                                $statusMap = [
        ...
        <aside class="xl:col-span-4 space-y-5">
            <div class="rounded-3xl overflow-hidden shadow-lg text-white"
                ...
                <div class="mx-5 mb-4 rounded-2xl bg-white p-4 text-center shadow-inner">
                    ...
                    <div class="mt-3 font-mono text-sm font-bold text-slate-800" dir="ltr">{{ $code ? '#'.$code : '—' }}</div>
                    <div class="text-xs text-slate-500 mt-1 truncate px-2">{{ $fullName }}</div>
                </div>
