...
@section('content')
    <div class="flex place-content-center">
        <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-md border-2 border-emerald-300" dir="rtl">
            ...
            <form method="POST" action="{{ route('place.update', $place->PlaceID) }}">
                @csrf
                @method('PATCH')
                        ...
                        <select id="location_id" name="location_id" required
                            ...
                            @foreach ($locations as $location)
                                <option value="{{ $location->LocationID }}"
                                    {{ $place->LocationID == $location->LocationID ? 'selected' : '' }}>
                    ...
                    <div class="flex justify-center">
                        <button type="submit"
                            class="inline-flex items-center justify-center h-12 px-8 text-sm font-medium tracking-wide 
                                   rounded-full bg-emerald-50 text-emerald-600 hover:bg-emerald-100 hover:text-emerald-700 transition">
                            تعديل المكان
                        </button>
                    </div>
