...
document.addEventListener('DOMContentLoaded', function() {
    ...
    seasonSelect.addEventListener('change', function() {
        ...
        fetch(`{{ route('media.getEventsForPages') }}?seasonID=${seasonId}`)
            ...
            .then(events => {
                ...
                if (events.length) {
                    events.forEach(event => {
                        ...
                        opt.textContent = `${event.EventName} (${event.EventStartDate} → ${event.EventEndDate})`;
            ...
            }).catch(() => eventSelect.innerHTML = '<option value="">خطأ في تحميل الفعاليات</option>');
    });
    ...
    loadMediaBtn.addEventListener('click', function() {
        ...
        fetch(`{{ route('media.getMediaForEvent') }}?seasonEventID=${selectedEvent.id}`)
            ...
            .then(data => {
                ...
                if (data.length) {
                    allMediaData = data.map(m => ({ ...m, type: detectMediaType(m.DriveLink) }));
    ...
    function filterMedia(type) {
        ...
        if(type==='photo') { filtered = allMediaData.filter(m=>m.type==='photo'); filterPhotos.classList.add('active'); }
        else if(type==='video') { filtered = allMediaData.filter(m=>m.type==='video'); filterVideos.classList.add('active'); }
    ...
    function displayMedia(mediaData) {
        ...
        mediaData.forEach((media, idx) => {
            if(media.DriveLink) mediaGrid.appendChild(createMediaCard(media.DriveLink, media.type, idx+1));
        });
    ...
    window.openMedia = function(link, type, title) {
        ...
        if(type==='video' && extractFileId(link)) {
            videoContainer.innerHTML = `<iframe src="https://drive.google.com/file/d/${extractFileId(link)}/preview" width="100%" height="100%" allow="autoplay" frameborder="0" class="rounded"></iframe>`;
        } else if(type==='photo' && extractFileId(link)) {
            videoContainer.innerHTML = `<img src="https://drive.google.com/uc?export=view&id=${extractFileId(link)}" alt="${title}" class="w-full h-full object-contain rounded" />`;
        } else {
            videoContainer.innerHTML = `<div class="flex items-center justify-center h-full text-gray-500 text-center"><div class="text-6xl mb-4">${type==='video'?'🎬':'📷'}</div>لا يمكن عرض الملف هنا<br>يرجى فتحه في Drive</div>`;
        }
...
<style>
.filter-btn.active { @apply font-semibold ring-2 ring-blue-300; }
.media-card.photo { border-left: 4px solid #10b981; }
.media-card.video { border-left: 4px solid #8b5cf6; }
.media-card.folder { border-left: 4px solid #f59e0b; }
</style>
@endsection
