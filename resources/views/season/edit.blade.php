@extends('layouts.app' , ['pageTitle' => __('Edit season')])

@section('content')
<div class="flex place-content-center">
    <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-md border-2 border-green-300">
        <div class="mb-6 text-center">
            <h2 class="text-xl font-bold text-gray-800">تعديل بيانات الموسم</h2>
        </div>

        <form method="POST" action="{{ route('season.update', $season->SeasonID) }}">
            @method('PATCH')
            @csrf
            <div class="space-y-6">
                <!-- Season Name -->
                <div class="relative">
                    <input id="season_name" type="text" name="season_name"
                        ...
                        value="{{ old('season_name', $season->SeasonName) }}"
                ...
                <div class="flex justify-center">
                    <button type="submit"
                        class="inline-flex items-center justify-center h-12 px-8 text-sm font-medium tracking-wide rounded-full bg-green-50 text-green-500 hover:bg-green-100 hover:text-green-600 transition">
                        تحديث الموسم
                    </button>
                </div>
            </div>
